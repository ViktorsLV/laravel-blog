...
<div class="mb-4 bg-white p-6 pb-4 rounded-lg hover:shadow-md">
    ...
    <p><a href="{{route('posts.show', $post)}}"
            class="mb-2 font-bold text-2xl hover:text-purple-400">{{Str::of($post->title)->limit(60, ' (...)')}}</a>
    </p>
    ...
    <div class="text-right flex flex-row justify-end">
        ...
        <div>
            ...
            <form action="{{ route('posts.destroy', $post) }}" method="post">
                ...
                @method('DELETE')
                <button type="submit"
                    class="bg-red-500 text-white px-4 py-2 mr-2 rounded-3xl font-medium hover:bg-red-600">Delete</button>
            </form>
            @endif
        </div>
    </div>
